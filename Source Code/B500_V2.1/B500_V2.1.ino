/*  Filename: ??
 *  Description: Program for Control, IoT, and Sensing Subsystem
 *  Contributors:
 *    Danu Ihza Pamungkas
 *    Dhanurangga Al Fadh
 *    Kevin Naoko
...
// Pin Definition
// Right Side
#define batteryButtonR 33   //12
#define chargerSW1R 32      //gakepake
#define chargerSW2R 33      //gakepake
#define redR 19     //23     //19     //4
#define greenR 18 //23
#define blueR 23 //18

// Left Side
#define batteryButtonL 32   //14
#define chargerSW1L 23  // gapake
#define chargerSW2L 18  // gapake
#define redL 16//17 //16     //15   
#define greenL 4 //17   //0
#define blueL 17//4     //2

// Others
#define chargingModePin 14  //19
#define SDA 21
#define SCL 22
#define txBMS 10
#define rxBMS 9
#define txGSM 26    //25
#define rxGSM 27    //26
#define relayCharger 25    //27
#define triggerButton 13

#define delayChargingState 15 * 1000    // seconds * 1000
#define delayBatterySampling 5 * 60000  // minutes * 60000 
...
byte cmd_periodicBattInfoSampleR = 0;
byte isValid_initialBattInfoR = 0;
long t_lastSampleR = millis();
byte stat_initialChargeDelayR = 0;
...
byte cmd_periodicBattInfoSampleL = 0;
byte isValid_initialBattInfoL = 0;
long t_lastSampleL = millis();
byte stat_initialChargeDelayL = 0;
...
char syncEnableTopic[50];
char syncEnableTopicCallback[50];
...
{ 
    ...
    sprintf(syncEnableTopic, "sys/enableStatus/%d", PRODUCT_ID); 
    sprintf(syncEnableTopicCallback, "sys/enableStatusCallback/%d", PRODUCT_ID); 
    ...
    xTaskCreatePinnedToCore(BattInfoTask, "BattInfoTask", 10000, NULL, 1, NULL, 0);  
}
...
void MQTTLoop(void *parameter)
{
    ...
    { 
        ...
        if (initialUpload == 0 && client.connected() && gsmStatus == 1){  
            strcpy(stringHolder, blank);
            buildStringBatteryInfo(0, stringHolder);
            Serial.println(stringHolder);
            client.publish(topics[0], stringHolder);
        ...
        if  (initialUpload == 1 && initialSyncEnable == 0){
            client.publish(syncEnableTopic, "srand()");
        }
        
        if (initialUpload == 1 && initialSyncEnable == 1){
...
void BattInfoTask(void *parameter){
    for (;;)
    {
        ...
        if (cmd_periodicBattInfoSampleL == 1){
            unsigned short temp_SoHL;
            unsigned short temp_remainingCapacityL;
            unsigned short temp_totalCapacityL;
            double temp_SoCL;   
            
            Serial.println("Masuk command periodic data sampling L");

            Serial.println("OLD INFO:");
            Serial.println(SoHL);
            ...
            temp_SoHL = SoHL; 
            temp_remainingCapacityL = remainingCapacityL;
            temp_totalCapacityL = totalCapacityL;
            temp_SoCL = SoCL;

            byte battSW = digitalRead(batteryButtonL);
            // sampling data (copas dari bagian stateRetreiveSerialL. Update bagian ini juga bila ada perubahan di sana
            ...
            if (totalCapacityL > 105 && totalCapacityL < 5){
                // kondisi ini berarti data yang diparse masih salah
                if (isValid_initialBattInfoL == 0){
                    // cmd bersangkutan tidak diset 0, fungsi diulang terus hingga initial data didapatkan
                }
                else{
                    SoHL = temp_SoHL; 
                    remainingCapacityL = temp_remainingCapacityL;
                    totalCapacityL = temp_totalCapacityL;
                    SoCL = temp_SoCL; 
            
                    Serial.println("REFRESHED INFO:");
                    Serial.println(SoHL);
                    Serial.println(remainingCapacityL);
                    Serial.println(totalCapacityL);
                    Serial.println(SoCL);   
                
                    cmd_periodicBattInfoSampleL = 0;
                }
                
            }
            else{ 
                if (isValid_initialBattInfoL == 0){
                    isValid_initialBattInfoL = 1;
                    cmd_sendSlotL = 1;
                }
                cmd_periodicBattInfoSampleL = 0;
                isValid_initialBattInfoL = 1;
        ...
        if (cmd_periodicBattInfoSampleR == 1){
            unsigned short temp_SoHR;
            unsigned short temp_remainingCapacityR;
            unsigned short temp_totalCapacityR;
            double temp_SoCR;   
            
            Serial.println("Masuk command periodic data sampling R");
            ...
            temp_SoHR = SoHR; 
            temp_remainingCapacityR = remainingCapacityR;
            temp_totalCapacityR = totalCapacityR;
            temp_SoCR = SoCR;

            byte battSW = digitalRead(batteryButtonR);
            // sampling data (copas dari bagian stateRetreiveSerialR. Update bagian ini juga bila ada perubahan di sana
            while(!(battDataValid) && counter < MAX_RETRIEVE && battSW){
            ...
            if (totalCapacityR > 105 && totalCapacityR < 5){
                // kondisi ini berarti data yang diparse masih salah
                if (isValid_initialBattInfoR == 0){
                    // cmd bersangkutan tidak diset 0, fungsi diulang terus hingga initial data didapatkan
                }
                else{
                    SoHR = temp_SoHR; 
                    remainingCapacityR = temp_remainingCapacityR;
                    totalCapacityR = temp_totalCapacityR;
                    SoCR = temp_SoCR; 
                    ...
                    cmd_periodicBattInfoSampleR = 0;
                }
                
            }
            else{ 
                if (isValid_initialBattInfoR == 0){
                    isValid_initialBattInfoR = 1;
                    cmd_sendSlotR = 1;
                }
                cmd_periodicBattInfoSampleR = 0;
                isValid_initialBattInfoR = 1;
...
void stateRetrieveSerialL(){
    ...
    while(!(battDataValid) && counter < MAX_INITIAL_RETRIEVE && battSWL){
        battDataValid = readBattData('L');
        battSWL = digitalRead(batteryButtonL);
        counter++;
        vTaskDelay(1500 / portTICK_PERIOD_MS);
    ...
    // bila data baterai belum diperoleh, akan dicoba diambil kembali setelah masuk mode charging
    if (battDataValid == 0){
        isValid_initialBattInfoL = 0;
        // pada state charging, akan diset cmd_periodicBattInfoSampleL = 1;
    } 
    else{
        isValid_initialBattInfoL = 1;
    }
...
void initChargingL(){
    ...
    if (battSW == 1 && voltage > 40000)
    {
        if (chargeReady)
        {
            stat_initialChargeDelayL = 0;
            stateL = CHARGING;
            if (isValid_initialBattInfoL == 1){
                cmd_sendSlotL = 1; 
            }
        }
    }
...
void stateChargingL()
{
    ...
    if (battSWL == 1 && voltage > 40000)
    {
        ...
        // send periodic sampling command slot L
        if (millis() - t_lastSampleL > delayBatterySampling){
            t_lastSampleL = millis();
            cmd_periodicBattInfoSampleL = 1;
        }

        // bila initial battery data sudah berhasil didapatkan, send cmd ambil battery data
        if (isValid_initialBattInfoL == 0){
            cmd_periodicBattInfoSampleL = 1;
        }
    }
...
void stateRetrieveSerialR()
{
    ...
    // Sampai sini, data serial sudah diperoleh 
    while(!(battDataValid) && counter < MAX_INITIAL_RETRIEVE && battSW){
        battDataValid = readBattData('R');
        battSW = digitalRead(batteryButtonR);
        counter++;
        vTaskDelay(1500 / portTICK_PERIOD_MS);
    ...
    // bila data baterai belum diperoleh, akan dicoba kembali setelah masuk state charging
    if (battDataValid == 0){
        isValid_initialBattInfoR = 0;
        // pada state charging, akan diset cmd_periodicBattInfoSampleR = 1;
    } 
    else{
        isValid_initialBattInfoR = 1;
    }
...
void initChargingR(){
    ...
    if (battSW == 1 && voltage > 40000)
    {
        if (chargeReady)
        { 
            stat_initialChargeDelayR = 0;
            stateR = CHARGING;
            if (isValid_initialBattInfoR == 1){
                cmd_sendSlotR = 1; 
            }
        }
    }
...
void stateChargingR()
{
    ...
    if (stat_initialChargeDelayR == 0){ 
        vTaskDelay(delayChargingState / portTICK_PERIOD_MS); // Nanti diubah
        stat_initialChargeDelayR = 1;
    } 
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    if (battSWR == 1 && voltage > 40000)
    {
        ...
        // send periodic sampling command slot R
        if (millis() - t_lastSampleR > delayBatterySampling){
            t_lastSampleR = millis();
            cmd_periodicBattInfoSampleR = 1;
        }
...
void reconnectmqttserver()
{
    while (!client.connected())
    {
        ...
        if (client.connect(clientId.c_str()))
        {
            ...
            client.subscribe(syncEnableTopicCallback);
...
void callback(char *topic, byte *payload, unsigned int length)
{
    ...
    if (strcmp(topic,commandTopic) || strcmp(topic,syncEnableTopicCallback) ){
        ...
        // set initial sync to 1, untuk melanjutkan proses upload 
        Serial.println("HALO MASUK GAK KESINI?");
        if (strcmp(topic,syncEnableTopicCallback)){
            Serial.println("HALO MASUK GAK KESINI PART 2?");
            initialSyncEnable = 1;
        }
    }
