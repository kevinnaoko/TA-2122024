...
def display(deviceSelect):  
    ...
    s1data = [ [0]*lenIndex[0] for i in range(4)]
    s2data = [ [0]*lenIndex[1] for i in range(4)] 

    loopCount = 0
    for row in cur.execute("SELECT * FROM " + querySelect[0] + ' ORDER BY id DESC LIMIT 1'):  
        print(row)
    ...
    loopCount = 0
    for row in cur.execute("SELECT * FROM " + querySelect[1] + ' ORDER BY id DESC LIMIT 1'):  
