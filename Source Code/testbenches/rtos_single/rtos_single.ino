...
void setup()
{
    ...
    WiFi.begin("ERDE LANTAI 3","Hakade17");
    ...
    client.setServer("192.168.20.216", 1883);
    client.setCallback(callback);
     
    // Task Idle R and L 
    xTaskCreatePinnedToCore(MQTTLoop, "MQTTLoop", 10000, NULL, 1, NULL, 1);
...
void buildStringBatteryInfo(int battSelect, char *holder)
{
    ...
    if (battSelect == 0)
    {
        strcat(holder, serialNumberL);
    }
    else
    {
        strcat(holder, serialNumberR);
    }
    strcpy(stringHolder, blank);
    strcat(holder, sep);
